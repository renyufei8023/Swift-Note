...
/*:
Swift枚举:
Swift中的枚举比OC中的枚举强大，因为Swift中的枚举是一等类型，它可有像类和结构体一样增加属性和方法
格式:
enum Method{
    case 枚举值
}
...
//可以使用枚举类型变量或者常量接收枚举值
var m:Method = .Add
//注意：如果变量或者常量没有指定类型,那么前面必须加上该值属于那个枚举类型
var m1 = Method.Add

//利用Switch匹配
//注意：如果case中包含了所有的值，可以不写default，如果没有包含所有值，则必须写default
...
enum Method4:Double {
    case Add = 5.0,Sub = 6.0,Mul = 6.1
}
// rawValue代表将枚举值转换为原始值, 注意老版本中转换为原始值的方法名叫toRaw
print(Method4.Add.rawValue)
